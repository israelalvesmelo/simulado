...
public class Gabarito {
	...
	public Resposta retornaRespostaPeloNumeroQuestao(int numero) throws NotFoundException {
		...
		resposta = this.respostas.stream().filter(r -> r.getQuestao().getNumero() == numero).findAny().orElseThrow(
				() -> new NotFoundException("Resposta para a questão [" + numero + "] não foi encontrada"));
		return resposta;
	}
}
